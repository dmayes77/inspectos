                    ...
                    {selectedServiceIds.length > 0 && form.price && (
                      <div className="text-sm space-y-1 p-3 bg-muted/50 rounded-md">
                        <div className="flex justify-between">
                          ...
                          <span className="font-medium">${calculatePackageDiscount().totalIndividualPrice.toFixed(2)}</span>
                        </div>
                        <div className="flex justify-between">
                          <span className="text-muted-foreground">Package price:</span>
                          <span className="font-medium">${calculatePackageDiscount().packagePrice.toFixed(2)}</span>
                        </div>
                        <div className="flex justify-between pt-2 border-t">
                          <span className={calculatePackageDiscount().discount > 0 ? "text-green-600 font-medium" : "text-muted-foreground"}>
                            {calculatePackageDiscount().discount > 0 ? "Savings:" : "No discount"}
                          </span>
                          {calculatePackageDiscount().discount > 0 && (
                            <span className="text-green-600 font-medium">
                              ${calculatePackageDiscount().discount.toFixed(2)} (
                              {((calculatePackageDiscount().discount / calculatePackageDiscount().totalIndividualPrice) * 100).toFixed(0)}% off)
                            </span>
                          )}
                        </div>
                      </div>
