...
import { createErrorHandler, createSuccessHandler } from "@/lib/utils/errors";
...
export default function EditInspectionPage(props: { isNew?: boolean } = {}) {
  ...
  // Use clientsData directly instead of syncing state
  const clients = clientsData;
  const [showClientForm, setShowClientForm] = useState(false);

  // Get inspection by ID (TODO: Replace with API call)
  ...
  // Update selected IDs and types when inspection loads
  useEffect(() => {
    if (inspection) {
  ...
  const calculatedPrice = useMemo(() => {
    return calculateServiceTotal(selectedTypeIds, services);
  }, [selectedTypeIds, services]);

  // Parse address using utility function (memoized)
  const addressParts = useMemo(() => {
    return inspection ? parseAddress(inspection.address) : { street: "", city: "", state: "", zip: "" };
  }, [inspection]);
  ...
  const inspectorObj = useMemo(() => {
    return inspectors.find((i) => i.teamMemberId === selectedInspectorId);
  ...
  const handleSubmit = useCallback(
    async (e: React.FormEvent<HTMLFormElement>) => {
      ...
      const payload: Partial<Inspection> & { types?: string[] } = {
        address: `${toString(getString(raw.street))}, ${toString(getString(raw.city))} ${toString(getString(raw.state))} ${toString(
          getString(raw.zip)
        )}`.trim(),
    ...
    [selectedTypeIds, selectedClientId, selectedInspectorId, clientObj, inspectorObj, calculatedPrice, inspection, updateMutation, createMutation, router]
  );
              ...
              <CardContent className="space-y-4">
                  ...
                  {showClientForm ? (
                    <ClientInlineForm
                      onCreate={useCallback((client) => {
                        ...
                        // Note: In production, this would use a mutation to add the client
                        // For now, we'll just set the selected ID and close the form
                        setSelectedClientId(newClient.clientId);
                        setShowClientForm(false);
                        toast.success("Client created. Please refresh to see in list.");
                      }, [])}
                      onCancel={useCallback(() => setShowClientForm(false), [])}
                    />
                  ) : (
                    <Select
                      name="client"
                ...
                <div className="space-y-2">
                  ...
                  <Select
                    ...
                    required
                    onValueChange={useCallback((value: string) => setSelectedInspectorId(value), [])}
                  >
                    <SelectTrigger className="w-full">
                      ...
                      services.map((service) => {
                        ...
                        return (
                          <ServiceCheckbox
                            ...
                            service={service}
                            checked={checked}
                            onToggle={(serviceId, isChecked) => {
                              setSelectedTypeIds((ids) => (isChecked ? [...ids, serviceId] : ids.filter((id) => id !== serviceId)));
                            }}
                          />
                        );
                      })}
