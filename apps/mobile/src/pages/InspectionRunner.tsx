      ...
      subHeader={
        <IonToolbar>
          <IonSegment
            ...
            onIonChange={(e) => goToSection(parseInt(e.detail.value as string))}
            scrollable
          >
          ...
          <IonCard>
            ...
            <IonCardContent>
              ...
              <IonButton
                expand="block"
                fill="outline"
                onClick={() => addFinding(currentSection.id)}
                style={{ marginTop: 16 }}
              >
    ...
    <div style={{ marginBottom: 16, padding: 8, background: 'var(--ion-color-light)', borderRadius: 8 }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
        <IonLabel>
          ...
          {item.description && (
            <p style={{ color: 'var(--ion-color-medium)' }}>{item.description}</p>
          )}
        </IonLabel>
      ...
      {item.item_type === 'rating' && (
        <div style={{ display: 'flex', gap: 4, marginTop: 8 }}>
          {RATINGS.map((r) => (
            <IonButton
              key={r.rating}
              fill={localValue === r.rating.toString() ? 'solid' : 'outline'}
              size="small"
              onClick={() => handleRatingChange(r.rating)}
            >
              {r.rating}
            </IonButton>
          ))}
