...
export async function GET(_request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (error || !data) {
    return NextResponse.json({ error: { message: error?.message ?? "Agency not found." } }, { status: 404 });
  }
...
export async function PUT(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const { data, error } = await supabaseAdmin.from("agencies").update(updateData).eq("tenant_id", tenantId).eq("id", id).select().single();

  if (error || !data) {
    return NextResponse.json({ error: { message: error?.message ?? "Failed to update agency." } }, { status: 500 });
  }
