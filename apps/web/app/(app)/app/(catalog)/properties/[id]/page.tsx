...
export default function PropertyDetailPage() {
  ...
  const contactActionHref = ownerClient ? `/app/contacts/${ownerClient.id}` : undefined;
  ...
  const mainContent = (
    <div className="space-y-6">
      <ClientInfoCard
        ...
        emptyActionHref={`/app/properties/${propertyId}/edit`}
      />
              ...
              <div key={owner.propertyOwnerId} className="flex items-start gap-3 rounded-lg border p-3">
                <div className="flex-1">
                  <div className="flex items-center gap-2">
                    {owner.client ? (
                      <Link href={`/app/contacts/${owner.client.id}`} className="font-medium hover:underline">
                        {owner.client.name}
                      </Link>
                    ) : (
