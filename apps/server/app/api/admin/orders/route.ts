...
export async function POST(request: Request) {
  try {
    ...
    const tenantSlug = (payload as { tenant_slug?: string }).tenant_slug || (request as NextRequest).nextUrl?.searchParams.get('tenant');
    const supabase = createUserClient(accessToken);
    const { tenant, error: tenantError } = await resolveTenant(supabase, user.userId, tenantSlug);
    if (tenantError || !tenant) {
      return badRequest('Tenant not found');
    ...
    if (inspectionError) {
      ...
      await supabase.from("orders").delete().eq("id", order.id);
      return serverError(inspectionError.message, inspectionError);
    }
