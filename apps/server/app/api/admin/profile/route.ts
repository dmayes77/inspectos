...
const PROFILE_FIELDS = 'id, email, full_name, avatar_url, phone, bio, social_facebook, social_twitter, social_linkedin, social_instagram, city, state_region, country, postal_code';
...
export const GET = withAuth(async ({ supabase, user }) => {
  ...
  if (error) return serverError('Failed to fetch profile', error);
